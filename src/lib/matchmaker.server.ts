...
export function rebalance(pods: string[][]): string[][] {
  const stage1: string[][] = [];
  for (const p of pods) {
...
export function heuristicSplit(ids: string[]): string[][] {
  const shuffled = [...ids].sort(() => Math.random() - 0.5);
  return rebalance(splitToPods(shuffled));
}

export function buildClusters(attendees: Attendee[]): Cluster[] {
...
export async function diversifyClusterWithAI(
  cluster: Cluster,
  apiKey: string,
